...
from message import array_endpoint, params, scroll_command
from settings import headers, url_root
...
def season_and_review():
    print(
        "По желанию можете ввести доп. параметры. Нажмите Enter чтобы пропустить поле"
    )
    ...
    if count_elements:
        try:
            if 0 < int(count_elements) < 250:
                ...
                print(
                    "Введено неверное значение. Будет установлено значение по умолчанию"
                )
        except (ValueError, TypeError, KeyError):
            print("Введено неверное значение. Будет установлено значение по умолчанию")
    if movie_id:
...
def main():
    ...
    while True:
        ...
        status, response = request_factory.make_request(
            endpoint=endpoint, headers=headers, params=query_params
        )
